(function () {
  const data = window.MILO_GRADE3_SOURCEBOOK_DATA;
  ...
  function readSet() {
    try {
      return new Set(JSON.parse(localStorage.getItem('milo-grade3-source-read') || '[]'));
    } catch {
      return new Set();
    }
  }
  ...
  function saveReadSet(set) {
    localStorage.setItem('milo-grade3-source-read', JSON.stringify([...set]));
  }
    ...
    return `
      <section class="sourcebook-hero">
        <div class="sourcebook-hero-copy">
          <span class="sourcebook-label">NGUỒN LỚP 3 ĐẦY ĐỦ · ${esc(data.version)}</span>
          <h2>Toàn bộ 190/190 ảnh từ anh_tam(3).zip đã nằm trong app</h2>
          <p>Đủ 190 trang nguồn Lớp 3 được giữ trong app dưới dạng WebP chất lượng cao để giảm dung lượng. Không loại mục lục, trang Unit, bài đọc, bài nghe, từ vựng, ngữ pháp, viết, project hoặc trang phụ.</p>
        ...
        <div class="sourcebook-stats">
          ...
          <article><b>1</b><span>ZIP nguồn</span></article>
          <article><b>12</b><span>Unit Lớp 3</span></article>
          <article><b>${data.allPagesIncluded ? '190/190' : 'Cần kiểm tra'}</b><span>trang đã tích hợp</span></article>
        </div>
      </section>
      <section class="sourcebook-proof">
        <span>🛡️</span>
        <div><b>Quy tắc thay kiến thức Lớp 3</b><p>${esc(data.canonicalRule)} Mỗi trang có tên tệp và đường dẫn để kiểm kê; ${data.allPagesIncluded ? '190/190 trang đã được tích hợp.' : 'có trang cần kiểm tra lại.'}</p></div>
      </section>`;
  }
    ...
    return `
      <section class="sourcebook-toolbar">
        <div class="sourcebook-tabs" role="tablist">
          ...
          <button type="button" data-source-filter="all" class="${state.filter === 'all' ? 'active' : ''}">Tất cả 190 ảnh</button>
          <button type="button" data-source-filter="shared" class="${state.filter === 'shared' ? 'active' : ''}">Trang chung/phụ</button>
        </div>
        <label class="sourcebook-search"><span>🔎</span><input id="sourcebookSearch" value="${esc(state.query)}" placeholder="Tìm Unit hoặc tên ảnh nguồn…"></label>
      </section>`;
  }
  ...
  function cardsHtml(pages) {
    const read = readSet();
    if (!pages.length) {
      return `<div class="sourcebook-empty"><span>🔎</span><b>Không tìm thấy ảnh phù hợp</b><p>Hãy xóa từ khóa hoặc chọn “Tất cả 190 ảnh”.</p></div>`;
    }
      ...
      <div class="sourcebook-modal hidden" id="sourcebookModal" role="dialog" aria-modal="true" aria-label="Trình xem ảnh nguồn">
        <div class="sourcebook-modal-shell">
          ...
          <div class="sourcebook-viewer" id="sourcebookViewer">
            ...
            <div class="sourcebook-image-stage" id="sourcebookImageStage"><img id="sourcebookImage" alt="Ảnh nguồn lớp 3"></div>
  ...
  function render(options = {}) {
    ...
    state.query = '';
    return `
      <div class="grade3-sourcebook grade2-sourcebook" id="grade3Sourcebook">
        ${summaryHtml()}
        ${toolbarHtml()}
  ...
  function bind(options = {}) {
    ...
    if (!window.__miloGrade3SourcebookKeysBound) {
      window.__miloGrade3SourcebookKeysBound = true;
  ...
  window.MILO_GRADE3_SOURCEBOOK = { render, bind, openAsset, data };
})();
